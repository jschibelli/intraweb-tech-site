...
- **Env set but still no data in HubSpot:** In Vercel Logs, open the `POST /api/contact` entry and check **External APIs**. If `api.hsforms.com` or `api.hubapi.com` show **400**: (1) **Forms API** — use the form GUID from HubSpot → Marketing → Lead Capture → Forms → your form → Options → Form ID; form field names in code must match the form’s field internal names (email, firstname, lastname, companyname or company, phone, website, message). (2) **Contacts API** — the route uses default property names (`company` not `companyname`; message is stored in a Note, not a contact property). If you see **403** on Resend, fix the API key and “from” domain in Resend. If n8n shows **404**, use the production webhook URL from the workflow’s Webhook node and ensure the workflow is active.
