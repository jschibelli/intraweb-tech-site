...
> **Status Note**  
> The active PR automation stack now lives in `scripts/automation/` (see `pr-automation-unified.ps1`, `auto-configure-pr.ps1`, `assign-pr-agents.ps1`, `docs-updater.ps1`).  
> This guide remains for legacy utilities still under `scripts/pr-management/` (quality checker, monitoring, sprint tools).
...
### Active Automation (New Location)
...
> Use the scripts above for any new development. The sections below cover the remaining legacy helpers in this folder.
...
├── (migrated) automate-pr-unified.ps1 → scripts/automation/pr-automation-unified.ps1
├── (migrated) configure-pr-auto.ps1 → scripts/automation/auto-configure-pr.ps1
├── (migrated) assign-pr-agents.ps1 → scripts/automation/assign-pr-agents.ps1
...
2. **Run PR analysis**:
   ```powershell
   .\scripts\automation\pr-automation-unified.ps1 -PRNumber 150 -Action analyze -Detailed
   ```
...
2. **Configure with specific parameters**:
   ```powershell
   .\scripts\automation\auto-configure-pr.ps1 -PRNumber 150 -Priority P0 -Size L -App "Dashboard" -Area "Backend"
   ```

3. **Dry run to preview changes**:
   ```powershell
   .\scripts\automation\auto-configure-pr.ps1 -PRNumber 150 -DryRun
   ```
...
1. **Assign PRs to agents**:
   ```powershell
   .\scripts\automation\assign-pr-agents.ps1 -ProjectNumber "20" -Owner "jschibelli"
   ```

2. **Dry run to preview assignments**:
   ```powershell
   .\scripts\automation\assign-pr-agents.ps1 -ProjectNumber "20" -DryRun
   ```

3. **Export assignment report**:
   ```powershell
   .\scripts\automation\assign-pr-agents.ps1 -ProjectNumber "20" -ExportTo "assignment-report.md"
   ```
...
1. **Analyze and respond to CR-GPT comments**:
   ```powershell
   .\scripts\automation\pr-automation-unified.ps1 -PRNumber 150 -Action respond -AutoFix
   ```

2. **Generate responses without auto-fixing**:
   ```powershell
   .\scripts\automation\pr-automation-unified.ps1 -PRNumber 150 -Action respond
   ```

3. **Monitor PR for new comments**:
   ```powershell
   .\scripts\automation\pr-automation-unified.ps1 -PRNumber 150 -Action monitor
   ```
...
1. **Run complete automation**:
   ```powershell
   .\scripts\automation\pr-automation-unified.ps1 -PRNumber 150 -Action all -AutoFix -AutoCommit
   ```

2. **Monitor workflow progress**:
   ```powershell
   .\scripts\automation\pr-automation-unified.ps1 -PRNumber 150 -Action monitor -Detailed
   ```

3. **Generate workflow report**:
   ```powershell
   .\scripts\automation\pr-automation-unified.ps1 -PRNumber 150 -Action all -ExportTo "workflow-report.json"
   ```
...
```powershell
# Enable dry run mode
.\scripts\automation\pr-automation-unified.ps1 -PRNumber $PRNumber -AutoFix -DryRun
