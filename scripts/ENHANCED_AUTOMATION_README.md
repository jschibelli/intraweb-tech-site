...
**Before using any automation scripts, you must run the setup script:**

```powershell
.\scripts\configuration\setup-project.ps1
```

This configures your GitHub organization, projects, and field mappings. See [SETUP_GUIDE.md](../../SETUP_GUIDE.md) for detailed instructions.
...
- **Automatically sets issue type** (Feature, Bug, Task)
- **Automatically configures project fields** (Status, Priority, Size for Project 6)
...
**Usage**:
```powershell
# Basic usage with auto branch creation and field configuration
...
# Without project field configuration
.\scripts\issue-management\create-issue-enhanced.ps1 `
  -Title "Issue title" `
  -Body "Description" `
  -IssueType "Feature" `
  -ConfigureProjectFields:$false
```
...
- `-Project`: Project key/name or number (uses default from config if not specified)
- `-Assignee`: GitHub username
- `-BaseBranch`: Base branch for new branch (default: "develop")
- `-IssueType`: Issue type - Feature, Bug, or Task (uses default from config)
- `-Priority`: Priority - P0, P1, or P2 (uses default from config if not specified)
- `-Size`: Size - XS, S, M, L, or XL (uses default from config if not specified)
- `-Status`: Status - Backlog, Ready, In progress, In review, Done (uses default from config if not specified)
- `-Estimate`: Estimate in days (uses default from config if 0)
- `-CreateBranch`: Auto-create branch (default: $true)
- `-PushBranch`: Auto-push to remote (default: $true)
- `-ConfigureProjectFields`: Auto-configure project fields (default: $true)
- `-DryRun`: Preview without creating

**Note**: All field values (Priority, Size, Status, Estimate) use defaults from your project configuration if not specified. The script automatically loads your configuration and uses the appropriate project.
...
When creating issues, the script automatically:
- Loads your project configuration (requires setup first)
- Uses your configured default project (or specified project)
- Sets the **Issue Type** (Feature, Bug, Task) from your configuration
- Sets **Status** using default from config (or specified value)
- Sets **Priority** using default from config (or specified value)
- Sets **Size** if specified (or uses default from config)
- Sets **Estimate** (in days) if specified (or uses default from config)
- All fields are configured automatically - no manual steps needed!

**Configuration**: All field IDs, option IDs, and defaults come from your `project-config.json` file, which is created by running `.\scripts\configuration\setup-project.ps1`.

**Note**: Start date and Target date can be set manually in the project UI if needed.
