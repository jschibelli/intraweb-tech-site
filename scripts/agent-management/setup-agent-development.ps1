...
function Create-AgentBranches {
    ...
    foreach ($issue in $Issues) {
        try {
            # Validate branch naming convention for issue branches (feature/123-something, etc.)
            $branchUtils = Join-Path $PSScriptRoot "..\\branch-management\\branch-utils.psm1"
            if (Test-Path $branchUtils) {
                Import-Module $branchUtils -Force -Prefix BranchUtils
                BranchUtilsAssert-BranchName -BranchName $issue.Branch | Out-Null
            }
