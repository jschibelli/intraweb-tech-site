...
function Process-AgentAction {
    ...
    switch ($Action) {
        ...
        "complete" {
            # Agent has completed implementation - this should typically be followed by create-pr
            # For now, we'll set to "In review" as work is complete
            $success = Update-ProjectStatus -IssueNumber $IssueNumber -Status "In review" -AgentName $AgentName
            if ($success) {
                Write-Host " Agent $AgentName completed Issue #$IssueNumber - work complete" -ForegroundColor Green
            }
        }
        "create-pr" {
            # Agent has created a PR - work complete, now in review
            $success = Update-ProjectStatus -IssueNumber $IssueNumber -Status "In review" -AgentName $AgentName
            if ($success) {
                Write-Host " Agent $AgentName created PR for Issue #$IssueNumber - work complete, in review" -ForegroundColor Green
            }
        }
        "merge-pr" {
            # PR has been merged
