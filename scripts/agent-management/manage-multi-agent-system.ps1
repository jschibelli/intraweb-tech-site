...
# Branch naming convention helpers (prefixed to avoid clobbering shared util functions).
$branchUtilsPath = Join-Path $PSScriptRoot "..\branch-management\branch-utils.psm1"
...
function Create-AgentWorkTree {
    ...
    # Validate non-issue branch name (e.g. feat/frontend/agent-setup).
    if (Get-Command BranchUtilsAssert-BranchName -ErrorAction SilentlyContinue) {
        BranchUtilsAssert-BranchName -BranchName $branchName -Mode Any | Out-Null
    }
