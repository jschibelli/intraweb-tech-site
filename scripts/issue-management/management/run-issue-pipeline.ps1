...
# Usage: .\scripts\issue-management\management\continuous-issue-pipeline.ps1 [-MaxIssues <NUMBER>] [-Status <STATUS>] [-Priority <PRIORITY>] [-Watch] [-Interval <SECONDS>] [-DryRun]
...
$queueManagerPath = Join-Path $PSScriptRoot "manage-issue-queue.ps1"
...
function Get-ProjectIssues {
    param([string]$Status, [string]$Priority)
...
function Process-Issue {
    ...
    # Step 5: Update status to "In review" (work complete, PR created)
    Write-ColorOutput "  Step 5: Setting status to 'In review'" "Yellow"
    if (-not $DryRun) {
        Update-ProjectStatus -ProjectItemId $Issue.id -Status "In review"
    } else {
        Write-ColorOutput "    [DRY RUN] Would set status to 'In review'" "Cyan"
    }
...
function Start-WatchMode {
    param([string]$Status, [string]$Priority, [int]$Interval, [int]$MaxIssues)
    ...
    while ($true) {
        try {
            ...
            $issues = Get-ProjectIssues -Status $Status -Priority $Priority
...
if ($Watch) {
    Start-WatchMode -Status $Status -Priority $Priority -Interval $Interval -MaxIssues $MaxIssues
    exit 0
}
