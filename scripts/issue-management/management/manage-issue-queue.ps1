...
# Import AI services
$aiServicesPath = Join-Path $PSScriptRoot "..\..\automation\core-utilities\ai-services.ps1"

if (Test-Path $aiServicesPath) {
    . $aiServicesPath
    $script:aiServicesAvailable = $true
} else {
    Write-Warning "AI services not found at $aiServicesPath - AI features will be disabled"
    $script:aiServicesAvailable = $false
}
...
function Get-ProjectFieldValue {
    ...
    try {
        $graphqlResponse = gh api graphql -f query='query($itemId: ID!) { node(id: $itemId) { ... on ProjectV2Item { fieldValues(first: 20) { nodes { ... on ProjectV2ItemFieldSingleSelectValue { field { ... on ProjectV2Field { id name } } name } ... on ProjectV2ItemFieldTextValue { field { ... on ProjectV2Field { id name } } text } ... on ProjectV2ItemFieldNumberValue { field { ... on ProjectV2Field { id name } } number } } } } } }' -f itemId="$ProjectItemId"
        $jsonData = $graphqlResponse | ConvertFrom-Json
...
function Initialize-QueueManager {
    ...
    if ($script:queueConfig.EnableAI) {
        if (-not $script:aiServicesAvailable) {
            Write-Warning "AI services not available. Continuing without AI features."
            $script:queueConfig.EnableAI = $false
        } elseif (-not $env:OPENAI_API_KEY) {
            Write-Warning "OPENAI_API_KEY not set. AI features disabled."
            $script:queueConfig.EnableAI = $false
        }
    }
...
function Add-IssueToQueue {
    ...
    # Auto-initialize queues if not already initialized
    if ($script:queueStorage.Queues.Count -eq 0) {
...
"@
    ...
    try {
        $systemPrompt = "You are an expert project manager. Recommend the best queue for issue processing based on priority, complexity, and type. Respond with only the queue name (high-priority, standard, low-priority, features, or bugs)."
        $fullPrompt = "$systemPrompt`n`n$prompt"
        $response = Invoke-AICompletion -Prompt $fullPrompt
